...
- [ ] commitment-sourced 支出的類別在 Phase 4 結算時寫死為 `'承諾扣款'`（佔位）；Phase 6 需允許將這些支出**重新歸類**為正式類別（金額/帳戶仍受「需從 Commitment 操作」限制，僅類別可改）
- [ ] 修改金額時反向調整帳戶餘額
- [ ] 刪除時反向調整帳戶餘額
...
| 信用卡繳費 / 轉帳      | 在帳戶間搬錢的帳務交易（Ledger Type B）：繳卡費＝銀行 −X、信用卡未繳 −X 一個動作搞定；一般轉帳同理。**不是新支出**（支出在刷卡標記已扣款時已記）。目前缺口只能用「調整餘額」手動處理兩邊。Ledger Model 下實作為一筆 transfer，餘額由 view 自動推導      |
| 儲蓄池健康度（缺口）   | 把 DESIGN 四的 Pool Health 延伸到一般儲蓄池：卡片顯示「已存 vs 已承諾 缺口」（例：已存 15,000・已承諾 40,000・缺口 −25,000），攤出「還沒存夠就提前承諾購買」。承諾可綁儲蓄池，進度公式 current/target 不變，缺口為獨立視圖。與 Phase 8 Dashboard 一起做 |
